<#
本脚本当前仅适用于Windows8和Windows2012以上版本的Windows操作系统
1.在修改RDP端口的过程中，首先修改注册表“HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp”下“PortNumber”的键值为指定的端口，接下来重启Remote Desktop Services服务，并将该端口对应的防火墙入站请求添加到防火墙例外中。
2.在恢复默认RDP端口的过程中，首先将之前添加的防火墙例外清理掉，然后将注册表中对应键值的端口改回3389，最后重启Remote Desktop Services服务。
需要强调的是重启远程桌面服务的过程中远程桌面将会短时间中断，需要重新连接，并将该端口对应的防火墙入站请求添加到防火墙例外中，此处添加的仅仅是操作系统层面的防火墙例外，如果读者使用的是类似Windows Azure之类的虚拟化方案一定要对应的增加外部防火墙(安全组)的例外，否则修改后的RDP端口将会无法从外部连接
